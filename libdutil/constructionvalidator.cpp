...
SettingRule checkSettingRule(SettingRule sr)
{
  ...
  if (!sr.listOfPossibleValues.empty() && sr.defaultValue.isValid()) {
    D_ASSERT_MSG(
        checkPresenceInListOfAllowedValues(sr.listOfPossibleValues, sr.defaultValue.toString()),
 ...
 */
template <typename ListType>
std::string checkWareObjectAndReturnErrors(ListType const& wares, std::string const& key,
                                           std::string const& referredTypeName,
                                           WarelistRule const& warelistrule)
{
  if (referredTypeName != warelistrule.type) {
    return "ware rule for '" + warelistrule.key + "': type mismatch: should be '"
           + warelistrule.type
           + "' according to rule, but was passed a named reference to an object of type '"
           + referredTypeName + "'";
  }
  ...
  if (!wareData.first->second) {
    return "ware rule for '" + warelistrule.key + "': was passed empty ware pointer with ware key '"
           + key + "' in the construction data";
  }
  ...
  if (Factory::isRegisteredInterface(referredTypeName)) {
    if (!Factory::isRegisteredWithInterface(referredTypeName, concreteClassName)) {
      return "ware rule for '" + warelistrule.key
             + "': type mismatch: object should be an instance of a class derived from '"
             + warelistrule.type
    ...
    if (referredTypeName != concreteClassName) {
      return "ware rule for '" + warelistrule.key + "': type mismatch: should be '"
             + warelistrule.type + "' according to rule, but was passed an object of type '"
             + concreteClassName + "' in the construction data";
    }
  }
  return "";
...
template <typename ListType>
std::string checkWareListRecursivelyAndReturnErrors(ListType const& wares, std::string const& key,
                                                    std::string const& referredTypeName,
                                                    WarelistRule const& warelistrule)
{
  if (referredTypeName != warelistrule.type) {
  ...
  if (warelistrule.length != WarelistRule::lengthNotDefined
      && static_cast<label_t>(wareData.second) != warelistrule.length) {
    // clang-format off
  ...
  for (size_t i = 0; i < wareData.second; ++i) {
    if (!wareData.first->second) {
      return "ware rule for '" + warelistrule.key
             + "': was passed empty ware pointer with ware key '" + key
    ...
    if (Factory::isRegisteredInterface(referredTypeName)) {
      if (!Factory::isRegisteredWithInterface(referredTypeName, concreteClassName)) {
        return "ware list rule for '" + warelistrule.key + "': type mismatch: object '" + key
               + ConstructionData::seperator + std::to_string(i)
               + "' should be an instance of a class derived from '" + warelistrule.type
               + "' according to rule, but was passed an object of incorrect type '"
               + concreteClassName + "' in the construction data";
      }
    } else {
      if (referredTypeName != concreteClassName) {
        return "ware list rule for '" + warelistrule.key + "': type mismatch: object '" + key
               + ConstructionData::seperator + std::to_string(i) + "' should be '"
               + warelistrule.type + "' according to rule, but was passed an object of type '"
               + concreteClassName + "' in the construction data";
      }
    }
...
std::string checkSubobjectRecursivelyAndReturnErrors(
    ...
    ConstructionValidator const&
        subCV, /*ConstructionValidator reference to be used to validate the subobject cd*/
        ...
        warelistrule) /*a map containing all warelist rules, use key to identify the warelist rule for the subobject cd to be validated*/
{
  ...
  // clang-format off
    ...
    if (subData.second == 1 && warelistrule.length != WarelistRule::lengthNotDefined) {
...
std::string ConstructionValidator::checkSettingRuleKeyAndReturnErrors(ConstructionData const& cd,
                                                                      std::string const& key) const
{
  ...
  if (cdKey != keyList.cend()) {
    if (cd.s.value(key).isValid() && !hasSettingRule(key)) {
      return error
             = "Construction data settings key '" + key + "' does not match any SettingRule key.";
    }
  }
...
std::string ConstructionValidator::checkSubObjectAndReturnErrors(ConstructionData const& cd,
                                                                 std::string const& key) const
{
  ...
  if (warelistRules_.find(key) == warelistRules_.cend()) {
    return "There is no registered warelistrule for key '" + key
           + "'. It is not possible to validate the subobject.";
  }
  ...
  return checkSubobjectRecursivelyAndReturnErrors(cd.subObjectData, key, subobjectCV,
                                                  warelistRules_);
}

std::string ConstructionValidator::checkSubObjectListAndReturnErrors(ConstructionData const& cd,
                                                                     std::string const& key) const
{
  // cd is the superior ConstructionData object containing all subobject cds.
  // key identifies the warelistrule for the subobjects type in the ConstructionValidator map.
  ...
  if (warelistRules_.find(key) == warelistRules_.cend()) {
    return "There is no registered warelistrule for key '" + key
           + "'. Unable to validate the subobjects.";
  }
...
std::string ConstructionValidator::checkSharedWareAndReturnErrors(
...
std::string ConstructionValidator::checkSharedWareListAndReturnErrors(
    ConstructionData const& cd, std::string const& key, std::string const& referredTypeName) const
{
  if (warelistRules_.find(key) == warelistRules_.cend()) {
    return "There is no registered warelistrule for key '" + key
  ...
  return checkWareListRecursivelyAndReturnErrors(cd.sharedWares, key, referredTypeName,
                                                 warelistrule);
}

Variant ConstructionValidator::validateSettingRuleKeyAndReturnValue(ConstructionData const& cd,
                                                                    std::string const key) const
{
  std::string error;
...
ConstructionData const& ConstructionValidator::validateAndReturnSubObjectCD(
    ConstructionData const& cd, std::string const key) const
{
...
std::vector<ConstructionData const*> ConstructionValidator::validateAndReturnSubobjectCDs(
    ConstructionData const& cd, std::string const& key) const
{
    ...
    ConstructionData const& cd, std::string const& key, std::string const& referredTypeName) const
{
  ...
  auto variantWare
      = cd.wareSettings.value(key + ConstructionData::seperator + Utility::toString(0));
...
std::shared_ptr<const Ware> ConstructionValidator::validateSharedWareAndReturnPointer(
    ConstructionData const& cd, std::string const& key, std::string const& referredTypeName) const
{
  std::string error = checkSharedWareAndReturnErrors(cd, key, referredTypeName);
  if (!error.empty())
...
ConstructionValidator::validateSharedListAndReturnValues(ConstructionData const& cd,
                                                         std::string const& key,
