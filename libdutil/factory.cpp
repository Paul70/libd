...
bool Factory::isRegisteredWithInterface(const std::string interfaceName, const std::string concreteClassName)
{
    auto const map = DGLOBALS::getInterfaceMap();
    auto range = map->equal_range(interfaceName);
    for (auto it = range.first; it != range.second; ++it) {
...
void Factory::registerInterfaceName(const std::string interfaceName, const std::string concreteClassName)
{
...
void Factory::unregisterInterfaceName(const std::string interfaceName, const std::string concreteClassName)
{
    D_ASSERT(isRegisteredWithInterface(interfaceName, concreteClassName));
    auto map = DGLOBALS::getInterfaceMap();
    auto range = map->equal_range(interfaceName);
