...
TEST_F(FactoryInterfaceTests, getFactoryNameViaTypeSettings_withGoodConstructionData_works)
{
    ConstructionData cd
        = ConstructionData()
              .setParameter<Ware::Type>(TESTS::DerivedA::getClassName())
              .addSharedWare(
                  TESTS::Base::BaseWareAReference("baseWareA",
                                                  std::make_shared<TESTS::CompoundWareA>(
                                                      ConstructionData().setParameter<TESTS::CompoundWareA::LabelA>(42))))
              .addSubobject<TESTS::Base::BaseWareAInstance>(ConstructionData().setParameter<TESTS::CompoundWareA::LabelA>(43));

    ASSERT_EQ(TESTS::DerivedA::getClassName(), DUTIL::FactoryInterfaceDetail::getConcreteClassNameViaWareTypeSetting(cd));
}
...
TEST_F(FactoryInterfaceTests, newInstanceViaTypeSettings_withGoodConstructionData_works)
{
    ConstructionData cd = ConstructionData()
                              .setParameter<Ware::Type>(DerivedA::getClassName())
                              .setParameter<DerivedA::DerivedAInt>(42)
                              .addSharedWare(
                                  Base::BaseWareAReference("baseWareA",
                                                           std::make_shared<CompoundWareA>(
                                                               ConstructionData().setParameter<CompoundWareA::LabelA>(1))))
                              .addSubobject<Base::BaseWareAInstance>(ConstructionData().setParameter<CompoundWareA::LabelA>(-5));
